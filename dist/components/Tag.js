...
/* ========================================
   TAG COMPONENT
   ======================================== */
export const Tag = forwardRef(({ label, variant = 'default', size = 'md', icon: Icon, onRemove, onClick, disabled = false, className, ...props }, ref) => {
    const variants = {
    ...
    return (_jsxs(Component, { ref: ref, whileHover: isClickable ? { scale: 1.02 } : undefined, whileTap: isClickable ? { scale: 0.98 } : undefined, onClick: isClickable ? onClick : undefined, className: cn("inline-flex items-center justify-center rounded-md border font-medium transition-colors", variants[variant], sizes[size].container, isClickable && "cursor-pointer", disabled && "opacity-50 cursor-not-allowed", className), children: [Icon && _jsx(Icon, { className: sizes[size].icon }), _jsx("span", { className: "leading-none", children: label }), onRemove && !disabled && (_jsx("button", { onClick: (e) => {
                    e.stopPropagation();
                    onRemove();
                }, className: "rounded hover:bg-black/10 transition-colors focus:outline-none p-0.5 -mr-0.5", "aria-label": "Remove", children: _jsx(X, { className: sizes[size].removeButton }) }))] }));
});
Tag.displayName = 'Tag';
