...
import { MessageSquare, Heart, MoreHorizontal, Reply, Trash2, Edit2, CornerDownRight } from 'lucide-react';
...
const CommentItem = ({ comment, currentUser, depth = 0, onReply, onLike, onEdit, onDelete, }) => {
    ...
    return (_jsxs("div", { className: cn("group relative", depth > 0 && "mt-4"), children: [_jsxs("div", { className: "flex gap-3", children: [_jsxs(Avatar, { className: "w-8 h-8 flex-none border border-border-primary/50", children: [_jsx(AvatarImage, { src: comment.author.avatarSrc, alt: comment.author.name }), _jsx(AvatarFallback, { children: comment.author.initials || comment.author.name.charAt(0) })] }), _jsxs("div", { className: "flex-1 min-w-0 space-y-1.5", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-sm font-semibold text-text-primary", children: comment.author.name }), _jsx("span", { className: "text-xs text-text-tertiary", children: comment.timestamp }), comment.isEdited && (_jsx("span", { className: "text-[10px] text-text-tertiary italic", children: "(edited)" }))] }), (isAuthor || onReply) && (_jsx(Dropdown, { trigger: _jsx(Button, { variant: "ghost", size: "sm", className: "h-6 w-6 p-0 opacity-0 group-hover:opacity-100 transition-opacity data-[state=open]:opacity-100", children: _jsx(MoreHorizontal, { className: "w-4 h-4 text-text-tertiary" }) }), children: _jsxs("div", { children: [onReply && (_jsxs(DropdownItem, { onSelect: () => setIsReplying(!isReplying), children: [_jsx(Reply, { className: "w-4 h-4 mr-2" }), "Reply"] })), isAuthor && onEdit && (_jsxs(DropdownItem, { onSelect: () => setIsEditing(true), children: [_jsx(Edit2, { className: "w-4 h-4 mr-2" }), "Edit"] })), isAuthor && onDelete && (_jsxs(_Fragment, { children: [_jsx(DropdownSeparator, {}), _jsxs(DropdownItem, { className: "text-status-error focus:text-status-error", onSelect: () => onDelete(comment.id), children: [_jsx(Trash2, { className: "w-4 h-4 mr-2" }), "Delete"] })] }))] }) }))] }), isEditing ? (_jsxs("div", { className: "space-y-2", children: [_jsx(Textarea, { value: editContent, onChange: (e) => setEditContent(e.target.value), className: "min-h-[80px] text-sm", autoFocus: true }), _jsxs("div", { className: "flex gap-2 justify-end", children: [_jsx(Button, { size: "sm", variant: "ghost", onClick: () => setIsEditing(false), children: "Cancel" }), _jsx(Button, { size: "sm", variant: "primary", onClick: handleEditSubmit, children: "Save" })] })] })) : (_jsx("div", { className: "text-sm text-text-secondary leading-relaxed break-words", children: comment.content })), !isEditing && (_jsxs("div", { className: "flex items-center gap-4 pt-1", children: [onLike && (_jsxs("button", { onClick: () => onLike(comment.id), className: cn("flex items-center gap-1.5 text-xs font-medium transition-colors", comment.isLiked
                                            ? "text-status-error"
