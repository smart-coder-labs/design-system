...
import { describeArc, formatValue } from './shared/chartUtils';
...
export const PieChart: React.FC<PieChartProps> = ({
  ...
  const slices = useMemo(() => {
    ...
    return data.map((item, i) => {
      ...
      return {
        ...
        path: describeArc(cx, cy, radius, startAngle, endAngle === 360 ? 359.99 : endAngle),
      };
    });
  ...
  const ariaLabel = title
    ? `${title} pie chart with ${data.length} slices`
    : `Pie chart with ${data.length} slices`;
