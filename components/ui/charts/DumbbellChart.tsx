...
export interface DumbbellChartDataPoint {
  label: string;
  start: number;
  end: number;
  color?: string;
...
export const DumbbellChart: React.FC<DumbbellChartProps> = ({
  ...
  const dotRadius = useMemo(
    () => Math.max(3, Math.min(6, (chartHeight / data.length) * 0.2)),
  ...
  const ariaLabel = title
    ? `${title} dumbbell chart with ${data.length} items`
    : `Dumbbell chart with ${data.length} items`;
      ...
      <div ref={containerRef} className="relative w-full">
        ...
        >
          ...
          <motion.g variants={containerVariants} initial={animated ? 'hidden' : undefined} animate={animated ? 'visible' : undefined}>
            {data.map((item, i) => {
              const y = getRowY(i);
              const sx = scaleX(item.start);
              const ex = scaleX(item.end);
        ...
        {showTooltip && interactive && hoveredIndex !== null && (
          <ChartTooltip
            ...
            items={[
              ...
              { label: 'Difference', value: formatValue(data[hoveredIndex].end - data[hoveredIndex].start) },
            ]}
          />
        )}
      </div>
