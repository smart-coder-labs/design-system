...
export const ScatterPlot: React.FC<ScatterPlotProps> = ({
  ...
  const { xTicks, yTicks, xMin, xMax, yMin, yMax } = useMemo(() => {
    ...
    const xt = generateTicks(rawXMin, rawXMax, 5);
    const yt = generateTicks(rawYMin, rawYMax, 5);
    return {
      xTicks: xt,
      yTicks: yt,
      xMin: xt[0],
  ...
  const legendItems = useMemo(
    () => data
      .filter((d) => d.label)
      .map((d, i) => ({ label: d.label!, color: getColor(d, i) })),
    [data, getColor],
  );

  const ariaLabel = title
    ? `${title} scatter plot with ${data.length} points`
    : `Scatter plot with ${data.length} points`;
