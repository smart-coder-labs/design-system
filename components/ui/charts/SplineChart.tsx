...
export const SplineChart: React.FC<SplineChartProps> = ({
  ...
  // Smooth spline paths using Catmull-Rom interpolation
  const seriesPaths = useMemo(() => {
    return seriesPoints.map((points) => catmullRomSpline(points, tension));
  }, [seriesPoints, tension]);
  ...
  const ariaLabel = title
    ? `${title} spline chart with ${series.length} series and ${data.length} data points`
    : `Spline chart with ${series.length} series and ${data.length} data points`;
