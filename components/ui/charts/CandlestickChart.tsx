...
export const CandlestickChart: React.FC<CandlestickChartProps> = ({
  ...
  const candleWidth = useMemo(
    () => Math.max(4, (chartWidth / data.length) * 0.6),
  ...
  const xAxisTicks = useMemo(
    () => data.map((d, i) => ({ position: getCandleX(i), label: d.label })),
    [data, getCandleX],
  );
  ...
  const ariaLabel = title
    ? `${title} candlestick chart with ${data.length} candles`
    : `Candlestick chart with ${data.length} candles`;
            ...
            {data.map((item, i) => {
              ...
              return (
                <motion.g
                  ...
                  style={{ transformOrigin: `${cx}px ${scaleY((item.open + item.close) / 2)}px` }}
