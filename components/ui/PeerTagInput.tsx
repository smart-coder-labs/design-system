...
export const PeerTagInput: React.FC<PeerTagInputProps> = ({
    ...
    return (
        <div className={cn("relative w-full max-w-sm", className)}>
            <div className={cn(
                "relative flex items-center bg-white dark:bg-zinc-900 rounded-2xl border-2 transition-all duration-200 overflow-hidden shadow-sm",
                isFocused
                ...
                <AnimatePresence>
                    {query && (
                        <motion.button
                            ...
                            className="p-2 mr-2 bg-zinc-100 dark:bg-zinc-800 rounded-xl hover:bg-zinc-200 dark:hover:bg-zinc-700 transition"
                        >
            ...
            <AnimatePresence>
                {isFocused && query && (
                    <motion.div
                        ...
                        className="absolute top-full left-0 w-full mt-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-xl z-50 overflow-hidden"
                    >
                        {filteredContacts.length > 0 ? (
                            <div className="p-2 space-y-1">
                                <div className="px-3 py-2 text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 flex items-center gap-1">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-1.5">
                                                    <span className="font-bold text-zinc-900 dark:text-zinc-100">{contact.name}</span>
                                                    {contact.isVerified && (
                                                        <BadgeCheck className="w-4 h-4 text-emerald-500 fill-emerald-100 dark:fill-emerald-900/30" />
                                                    )}
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <span className="text-sm font-semibold text-zinc-500 dark:text-zinc-400">{contact.tag}</span>
                            ...
                            <div className="p-6 text-center text-zinc-500 dark:text-zinc-400 font-medium">
