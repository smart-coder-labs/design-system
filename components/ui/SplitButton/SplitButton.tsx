...
export const SplitButton = forwardRef<HTMLDivElement, SplitButtonProps>(
    (
        ...
        // Opens the menu (if needed) and moves the roving index to the first/last enabled item.
        const openMenuAt = (direction: 1 | -1) => {
            const next = findEnabledIndex(-1, direction);
            const alreadyActive = isOpen && next >= 0 && next === activeIndex;
            setIsOpen(true);
            setActiveIndex(next);
            // The focus effect only reacts to state changes, so re-focus explicitly when the
            // target index is unchanged (e.g. Shift+Tab back to the trigger, then ArrowDown).
            if (alreadyActive) itemRefs.current[next]?.focus();
        };
